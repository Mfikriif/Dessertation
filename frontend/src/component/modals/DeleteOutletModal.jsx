...
const DeleteOutletModal = ({ isOpen, onClose, onDelete, outletData }) => {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await onDelete(outletData.id_outlet);
      onClose();
    } catch (error) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col items-center text-center p-8">
        ...
        <p className="text-gray-400 mt-2 text-sm">
          Menghapus outlet mungkin berakibat pada hilangnya data stok pada outlet ini.
        </p>
