...
const EditBahanBakuModal = ({ isOpen, onClose, onEdit, initialData }) => {
  ...
  const validate = () => {
    ...
    if (!formData.nama_bahan.trim())
      newErrors.nama_bahan = "Nama bahan baku wajib diisi";
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await onEdit(initialData.id_bahan_baku, formData);
    if (result.success) {
      onClose();
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden">
        <div className="flex justify-between items-center p-6 border-b border-gray-100">
          <h2 className="text-xl font-bold text-gray-900">Edit Bahan Baku</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            />
            {errors.nama_bahan && (
              <p className="text-red-500 text-xs mt-1">{errors.nama_bahan}</p>
            )}
          </div>
          ...
          <div className="flex gap-4">
            ...
            <div className="w-1/2">
              ...
              <input
                type="text"
                disabled
                className="w-full px-4 py-2 border rounded-xl outline-none transition-colors bg-gray-100 text-gray-500 cursor-not-allowed"
                value={initialData?.stok_minimum || ""}
              />
              <p className="text-gray-400 text-xs mt-1">
                Hanya dapat dibaca (ReadOnly)
              </p>
            </div>
          </div>
...
export default EditBahanBakuModal;
